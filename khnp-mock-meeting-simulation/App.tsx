...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    // Firebase 실시간 구독
    const unsubscribe = stateService.subscribeToRooms((updatedRooms) => {
  ...
  const handleSave = async () => {
    ...
    if (editRoom) {
      await stateService.updateRoom(editRoom.id, formData);
      setEditRoom(null);
    } else {
      await stateService.createRoom(formData.name, formData.teamCount, formData.duration);
    }
              ...
              <div className="flex flex-col md:flex-row justify-between md:items-center border-b-2 border-gray-100 pb-4 mb-4 gap-4">
                ...
                <div className="flex gap-2 flex-wrap">
                  <NeoButton
                    ...
                    disabled={room.isStarted || (room.participants || []).length < 1}
        ...
        {step === 3 && selectedRoom && (
          <div className="space-y-6">
            ...
            <input
              ...
              disabled={isJoining}
            />
            <NeoButton className="w-full py-4 text-xl" onClick={handleJoin} disabled={isJoining}>
              {isJoining ? '입장 중...' : '회의실 입장'}
            </NeoButton>
            <button onClick={() => setStep(2)} className="w-full font-bold text-gray-500" disabled={isJoining}>팀 다시 선택</button>
          </div>
        )}
        ...
        <div className="bg-white neo-border neo-shadow p-12 text-center max-w-lg w-full">
          ...
          <div className="bg-gray-50 neo-border-sm p-4 text-left">
            ...
            <div className="flex flex-wrap gap-2">
              {participants.filter(p => p.teamIndex === me.teamIndex).map(p => (
